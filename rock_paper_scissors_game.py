...
def get_player_choice():
    """Get user input and validate it is one of the choice above"""
    player_choice = None
    while player_choice is None:
        player_choice=input('Choices: \n(R)ock \n(P)aper \n(S)cissors \n\nPick: ')
        if player_choice.lower() not in CHOCIES:
            player_choice=None
...
def get_computer_choice():
    ...
    computer_choice = CHOCIES[computer_choice]
...
def play_game():
    ...
    if is_draw(player_choice,computer_choice):
        print("It's a draw , both player picked %s: " % player_choice)    
    else:
        print("Computer picked: %s" % computer_choice)
        print("palyer picked: %s" % player_choice)
